...
// What Groq extracts — source is added by us after normalization
interface GroqJob {
  title: string;
  institution: string;
...
interface NormalizedJob extends Omit<GroqJob, "isJobPosting"> {
  source: string;
}
...
// Fetches a job detail page; returns null if the page is definitively gone/removed.
async function fetchJobPage(url: string): Promise<string | null> {
  ...
  try {
    ...
    return ""; // network error — keep item, just no description
  }
...
async function fetchHtml(
  ...
  // For individual job pages, extract <main> and strip markup
  const mainMatch = html.match(/<main[^>]*>([\s\S]*?)<\/main>/i);
  const content = mainMatch ? mainMatch[1] : html;
  return content
    .replace(/<script[\s\S]*?<\/script>/gi, "")
...
function parseHNet(html: string): RawItem[] {
  const items: RawItem[] = [];
  const articleRegex =
...
// Parses a standard WordPress RSS feed (handles both plain text and CDATA titles/links)
...
function parseNAPA(html: string): RawItem[] {
  const $ = load(html);
  const items: RawItem[] = [];
  const seen = new Set<string>();
  const BASE = "https://www.practicinganthropology.org";
  const EXCLUDED = /\/(purchase|advertise|post-a-job|submit|add-job|buy|pricing|packages|about|contact|resources|membership|events|news|donate|login|register|faq|policy|privacy|sitemap|guide|profile|member|directory|people|person)/i;
  ...
  $("main a[href], .field-item a[href], .view-content a[href], .entry-content a[href], article a[href], #content a[href]").each(
    (_, el) => {
      const $a = $(el);
...
function parseSfAA(html: string): RawItem[] {
  const $ = load(html);
  const items: RawItem[] = [];
  const seen = new Set<string>();
...
const SOURCES: Source[] = [
  ...
  },
  {
    ...
    name: "NAPA",
    urls: [
      "https://www.practicinganthropology.org/mentoring-career/position-listings/",
    ],
    parse: parseNAPA,
  },
  {
...
Use an empty string for unknown non-nullable string fields. Prefer a reasonable inference over null or empty string whenever the text gives any basis for one.`,
    ...
    // Guard: model sometimes returns [[...]] instead of [...]
    if (Array.isArray(parsed) && parsed.length > 0 && Array.isArray(parsed[0])) {
      parsed = parsed.flat(1);
    }
    return (parsed as unknown[]).filter(
      (item): item is GroqJob => item !== null && typeof item === "object" && !Array.isArray(item),
    );
  } catch (err) {
    if (err instanceof Groq.APIError && err.status === 429) {
      if (retries > 0) {
        ...
        console.warn(`[groq] Rate limited — waiting ${waitSecs}s then retrying (${retries} left)`);
        await sleep(waitSecs * 1000);
        return normalizeBatch(client, items, retries - 1);
      }
      console.warn(`[groq] Rate limit retries exhausted — skipping batch of ${items.length} items: ${items.map((i) => i.title).join(", ")}`);
      return [];
    }
    if (err instanceof Groq.APIError && err.status === 413) {
      if (items.length > 1) {
        console.warn(`[groq] Request too large (${items.length} items) — splitting in half`);
...
async function scrapeSource(
  ...
  // Fetch full job page text, 1 s apart; drop items whose page is gone/unavailable
  const fetchedItems: RawItem[] = [];
  for (let i = 0; i < rawItems.length; i++) {
    if (i > 0) await sleep(1000);
    const content = await fetchJobPage(rawItems[i].link);
    if (content === null) {
      console.log(`[scrape] ${source.name}: skipping unavailable page: ${rawItems[i].link}`);
      continue;
    }
  ...
  // Normalize in batches of 5
  const BATCH_SIZE = 5;
  const normalized: NormalizedJob[] = [];
  for (let i = 0; i < rawItems.length; i += BATCH_SIZE) {
    const results = await normalizeBatch(groqClient, rawItems.slice(i, i + BATCH_SIZE));
...
async function scrapeExa(groqClient: Groq): Promise<NormalizedJob[]> {
  const exa = new Exa(process.env.EXA_API_KEY!);
  const seen = new Set<string>();
  const rawItems: RawItem[] = [];

  // Run queries 3 at a time
  const CHUNK_SIZE = 3;
  ...
  // Drop obvious non-job pages by URL before hitting Groq
  const EXA_EXCLUDED_URL = /\/(people|person|faculty|staff|about|news|events|blog|publications|research|guide|profile|member|directory)(\/|$)/i;
...
export async function GET(request: Request) {
  try {
    ...
    // Map camelCase → snake_case for the DB; isJobPosting already stripped upstream
    const dbRows = liveJobs
      .map(({ sharingRestricted, ...rest }) => ({
        ...rest,
        sharing_restricted: sharingRestricted ?? false,
      }))
      // Drop any row that isn't a plain object with a string url (catches array bleed-through)
